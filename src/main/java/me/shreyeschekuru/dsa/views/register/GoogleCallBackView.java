...
@Route(value = "oauth2/callback/google")
@AnonymousAllowed
public class GoogleCallBackView extends VerticalLayout implements BeforeEnterObserver {
    ...
    public GoogleCallBackView(GoogleAuthService googleAuthService, UserService userService) {
    ...
    public void beforeEnter(BeforeEnterEvent beforeEnterEvent) {
        System.out.println("Entering the beforeEvent of sub class GoogleCallBack 2.0 ");
        ...
        if (code != null) {
            ...
            String idToken = googleAuthService.exchangeCodeForGoogleIdToken(code);
            if (idToken != null) {
                User userDto = googleAuthService.getUserInfoFromGoogleIdToken(idToken);
                handleUserRegistration(userDto, "google");
            } else {
                System.out.println("Google Token Failure");
            }
        } else {
    ...
    private void handleUserRegistration(User userDto, String provider) {
        if (userDto != null) {
            ...
            if (existingUser == null) {
                ...
                if (!existingUser.getoAuthProvider().equalsIgnoreCase("google"))
                {
